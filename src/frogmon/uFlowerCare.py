...
class FLOWERCARE:
	...
	def encodeStatJson(self, fileNM, data):
		...
		try:
			...
			dict['CDS']       = data[7]
			dict['BATTERY']   = data[8]
			dict['OUT_TEMP']  = data[9]
			dict['OUT_HUMI']  = data[10]
			dict['OUT_WIND']  = data[11]
			dict['OUT_RAIN']  = data[12]
			dict['OUT_SKY']   = data[13]
			dict['OUT_BATT']  = data[14]
			dict['LASTUPDT']  = data[15]
			dict['TEAMVIEWER'] = data[16]
		...
		while True:
			...
			for [flora_name, flora] in self.flores.items():
				...
				try:
					...
					datas.append(GLOB.readConfig(configFileNM, 'WEATHER', 't1h', '0')) # OUT TEMP
					datas.append(GLOB.readConfig(configFileNM, 'WEATHER', 'reh', '0')) # OUT HUMI
					datas.append(GLOB.readConfig(configFileNM, 'WEATHER', 'wsd', '0')) # OUT WIND
					datas.append(GLOB.readConfig(configFileNM, 'WEATHER', 'pty', '0')) # OUT RAIN
					datas.append(GLOB.readConfig(configFileNM, 'WEATHER', 'sky', '0')) # sky
					datas.append('0')                                    # OUT BATTERY
				...
				if rc == 0:
					if GLOB.appendControlInfo(COM.gJsonDir+"device.json", GLOB.readConfig(controlFileNM, 'CONTROL', 'remote', '0'), GLOB.readConfig(controlFileNM, 'CONTROL', 'group1', '0'), GLOB.readConfig(controlFileNM, 'CONTROL', 'group2', '0')) == 0 :
